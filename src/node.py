...
class Node(NodeServicer):
    ...
    def __init__(self,n:int,node_id: int, port: int) -> None:
        print(f'creando nodo con node_id: {node_id}')
        self.n: int = n
...
# cada entrada (node_id +2î mod 2^n) apunta a la cola, en orden reverso
    ...
    def Lookup(self, request, context) -> node_pb2.Empty:
        ...
        print(f'pasando por el nodo {self.node_id}')
        key: str = request.key
    ...
    def Insert(self, request, context) -> node_pb2.Empty:
        ...
        if self.is_id_at_current_node(hash_):
            ...
            print(f'Insertando valor "{value}" identificado por la clave "{key}", con hash {hash_}, numero del nodo {self.node_id}')
        ...
        print(f'buscando el hash {hash_}')
    ...
    def init_rpc_server(self):
        ...
        server.add_insecure_port(self,server)
        server.start()
        server.wait_for_termination()

    """Metodos para el uso del chord"""
    def init_finger_table(self) -> None:
        for i in range(self.n.bit_length() -1):
            ...
            finger_port: int = (self.por - self.node_id + finger_entry) 
    ...
    def is_id_at_current_node(self,id_:int) -> bool:
        return self.predecessor_id < id_ <= self.node_id
